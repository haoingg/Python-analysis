...
(coef_r <- coef(model)) #coef() 두 개 독립변수에 의해 꺼내진 회귀계수(기울기값/y절편값)
coef_r[1] #전체 설명력
coef_r[2] #아빠 
coef_r[3] #엄마
...
#interval = "confidence" : 예측값 뿐만 아니라 상한값, 하한값 함께 추출



fdata2 <- read.csv("data/factory2.csv")
fdata2.lm <- lm(robber~temp+pressure, data=fdata2)
summary(fdata2.lm) #두 개 독립변수에 대한 각 회귀계수 정보가 나온 회귀계수가가 얼만큼의 영향력을 주는지(유의미한지)
...
summary(mtcars)
cor(mtcars) #상관계수 (음의 상관관계-기름을 많이 쓰게 하는 애들이라 해석)
...
#mpg:종속변수, 나머지:독립변수 / 독립변수 간 상관관계가 높으면 <다중공선성> 문제를 일으키기 쉬움,
#모임으로써 전체적인 검정력을 높여줌
